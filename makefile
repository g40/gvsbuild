...
# do *not* change on command line with `make PROJECT=gtk4` for example
# this will break things in a subtle and hard to debug manner for 
# subsequent invocations of nmake in particular which will find a spurious
# '--' flag in its MAKEFLAGS env-var.
# PROJECT=zlib
PROJECT=librsvg
# release|debug
CONFIG=debug
...
BUILD_DIR=r:/src/gtk/$(PROJECT)_$(CONFIG)
...
build:
	...
	python starter.py build --verbose --ninja-opts -j=2 --build-dir $(BUILD_DIR) --log-single --capture-out --msys-dir  $(MSYS_DIR) --configuration $(CONFIG) $(PROJECT)
...
rebuild: 
	...
	python starter.py build --verbose --from-scratch --ninja-opts -j=2 --build-dir $(BUILD_DIR) --msys-dir $(MSYS_DIR) --log-single --capture-out --configuration $(CONFIG) $(PROJECT)

# install gvsbuild into the local Python cache
install:
	pip install .

# rebuild all using gvsbuild. can run in .venv
gvs:
	@echo "Building $(PROJECT) into $(BUILD_DIR)"
	pip install .
	-@rm $(BUILD_DIR)/logs/gvsbuild-log.txt
#	--from-scratch
	gvsbuild build --from-scratch --verbose --capture-out --ninja-opts -j=2  --build-dir $(BUILD_DIR) --msys-dir $(MSYS_DIR) --log-single --configuration $(CONFIG) $(PROJECT)
